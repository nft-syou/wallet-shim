...
追記（2026-09-19）: Orca 1.4.205 の内蔵ブラウザ（`orca tab create` → `orca eval` で `<script src>` 注入 → JS でクリック）でも、フィクスチャに対して `detected: MetaMask` → 接続 → 署名 → 送信 → 合成レシート（実 Sepolia RPC のブロック番号）まで `DONE` を確認。`orca snapshot` / `click` / `wait` は内部の agent-browser ヘルパーが stale で `browser_owner_unavailable` になったため使わなかった。手順は `skills/wallet-shim/recipes/orca-cli.md`。
